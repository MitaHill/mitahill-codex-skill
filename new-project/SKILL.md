...
- **新增能力先研究** — 新增模块、集成或通用能力时使用 `reuse-first`，同时检查项目内部、标准库、官方包仓库和 GitHub，再按项目体量、维护、安全、许可证及依赖成本选择复用或小型自研
...
按项目实际形态选择验证层级，不用 Docker、curl 或单元测试替代用户真正接触的运行界面：

1. 运行格式化、静态检查、类型检查和针对性自动化测试。
2. 使用项目规定的启动方式；采用 Docker 的项目再执行镜像构建、容器状态和日志检查。
3. 后端与 API 使用真实请求验证关键成功和错误路径。
4. Web 界面启动后使用 `runtime-verification`，通过浏览器或 Computer Use 操作关键流程，并检查控制台、网络、相关视口和视觉状态。
5. 桌面或移动图形程序必须启动真实应用或模拟器，通过 Computer Use 验证受影响流程并检查日志。
6. 修复发现的问题后重跑针对性检查，最后运行项目可承受的完整测试套件。

图形化程序仅完成构建、API 健康检查或自动化测试时，不得宣称验收完成。无法使用对应运行环境或控制工具时，明确报告未验证范围和原因。
