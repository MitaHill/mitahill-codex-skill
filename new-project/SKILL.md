...
- 初始化的标准流程：
  ...
  git init
  ```
- 初始化仓库不包含暂存或提交；通过 `git-workflow` 汇报完整状态并取得用户明确授权后，才能执行首次提交
...
- **独立脚本归位** — 运维、迁移、数据修复和一次性开发脚本放入小型 `scripts/` 目录；脚本保持薄入口并调用现有应用接口，不把脚本专用代码塞进应用主程序
...
4. **实现核心功能** — 迭代开发，每完成一个逻辑单元就验证并准备原子提交方案
5. **验证** — 确保功能正常运行
6. **总结** — 向用户说明项目结构、使用方法
...
6. 判断结果
   ├── 有错误 → 修复代码 → 回到步骤 1 重新测试
   └── 无错误 → 通知用户进行功能测试
```

## 12. 提交规范

优先遵循仓库已有规范；没有明确规范时使用以下 Conventional Commits 格式。未经用户通过 `git-workflow` 明确授权，不执行暂存或提交。
